...
export function useWeatherSearch() {
  const CB = useCallback(
    async (props: PUseWeatherSearch): Promise<TUseWeatherSearch> => {
      ...
      try {
        ...
        if (!geoData) throw Error("Location not found.");
        ...
        if (!wData) throw Error("Weather data unavailable for this location.");
